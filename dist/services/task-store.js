...
export class RedisTaskStore {
    ...
    async createTasksBulk(user_id, tasks) {
        const taskIds = [];
    ...
    async updateTask(user_id, task_id, updates) {
        const taskKey = this.taskKey(task_id);
        const task = await this.redis.hGetAll(taskKey);
        if (!task || Object.keys(task).length === 0)
            return false;
    ...
    async getTasksByFilters(user_id, statuses = ["pending", "completed"], start, end) {
        let taskIds = [];
